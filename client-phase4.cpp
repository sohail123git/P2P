...
// BEGIN phase 4
    ...
    // cout<<"no_files_to_search:"<<no_files_to_search<<endl;
    // cout<<"files_to_search ↓"<<endl;
    // for(string file:files_to_search){
    //     cout<<file<<", ";
    // }
    
    // cout<<endl;
    // cout<<"files_found_at_uid ↓"<<endl;
    // print_array<int>(files_found_at_uid, no_files_to_search);
    // cout<<"file_found_at_2hop_uid ↓"<<endl;
    // print_array<int>(file_found_at_2hop_uid, no_files_to_search);


    for(int j=0;j<no_files_to_search;j++){
        if(files_found_at_uid[j]>0){
            cout<<"Found "<<files_to_search[j]<<" at "<<files_found_at_uid[j]<<" with MD5 0 at depth 1"<<endl;
        }
        else if(file_found_at_2hop_uid[j]>0)
            cout<<"Found "<<files_to_search[j]<<" at "<<file_found_at_2hop_uid[j]<<" with MD5 0 at depth 2"<<endl;
